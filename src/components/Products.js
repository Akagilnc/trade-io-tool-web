    ...
    return (
      <div id="products">
        ...
        <Form>
          ...
          <Button variant="primary" onClick={this.submitSearch} type="button">
            提交
          </Button>
        </Form>
        {this.state.paginationNext ? (
...
function routeFormatter(cell) {
  let arr = cell.split('/');
  let length = arr.length;
  let product = arr[length - 2];
  //console.log(product);
  return (
    <Link to={`/product/${product}`} target="_blank">
      Edit
    </Link>
  );
}
